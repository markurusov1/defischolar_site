...
| **Collateral Valuation Example** | If your portfolio has $100K in stocks + hedging options, the current value might be $100K, but risk model shows max loss of $20K under stress → effective collateral value ~$80K (after offsets). Margin requirement: 15-30% of notional, allowing higher borrow. | For a DeFi LP position (e.g., Uniswap v3 NFT as collateral), the current value might be $10K, but IL stress test shows $3K potential loss → effective value $7K. LTV adjusts dynamically (e.g., 80% borrow if low risk). Not standard in all DeFi (e.g., Aave uses simpler cross-margin), but growing in derivatives DEXs. |
